...
def test_janapada_failure():
    ...
    if response.status_code == 200:
        ...
        assert not mock_amatya.call.called, "Amatya should not be called if Janapada fails"
...
def test_no_snippets_found():
    ...
    client = TestClient(app)
    test_query = {
        "question": "some very obscure question",
        "role": "developer"
    }
    headers = {"Authorization": "Bearer demo-token"}
    ...
    if response.status_code == 200:
        ...
        assert "couldn't find" in data['answer'].lower(), "Should indicate no results found"
