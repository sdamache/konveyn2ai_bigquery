...
class IRSParserImpl(IRSParser):
    ...
    def __init__(self, version: str = "1.0.0"):
        ...
        self.hash_generator = ContentHashGenerator(
            normalize_whitespace=True, ignore_comments=False
        )
        ...
        self.chunker = ContentChunker(
            ChunkConfig(
                max_tokens=1000, overlap_pct=0.15, strategy=ChunkingStrategy.FIXED_WIDTH
            )
        )
        ...
        self.patterns = {
            ...
            "layout_header": re.compile(
                r"IRS\s+.*Master\s+File.*Record\s+Layout", re.IGNORECASE
            ),
            "record_type": re.compile(
                r"Record\s+Type:\s*(.+?)\(Type\s+(\d+)\)", re.IGNORECASE
            ),
            "layout_version": re.compile(
                r"Layout\s+Version:\s*([^\s\n]+)", re.IGNORECASE
            ),
            ...
            "section_header": re.compile(
                r"^SECTION\s+([A-Z]):\s*([A-Z\s_]+)", re.IGNORECASE | re.MULTILINE
            ),
            "field_definitions_start": re.compile(
                r"Field\s+(?:Positions\s+and\s+)?Definitions:", re.IGNORECASE
            ),
            # Record length and other metadata
            "record_length": re.compile(
                r"RECORD\s+LENGTH:\s*(\d+)\s*bytes", re.IGNORECASE
            ),
        }
    ...
    def parse_imf_layout(self, layout_content: str) -> list[ChunkMetadata]:
        ...
        try:
            ...
            field_positions = self.extract_field_positions(
    ...
    def extract_field_positions(self, layout: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for match in field_matches:
            ...
            section_name = (
                section.strip()
                if section
                else self._infer_section_from_field(field_name)
            )
    ...
    def _infer_section_from_field(self, field_name: str) -> str:
        ...
        if any(
            term in field_lower
            for term in ["ssn", "social security", "name", "taxpayer"]
        ):
            return "IDENTITY"
        elif any(
            term in field_lower
            for term in ["tax", "income", "liability", "withh", "agi"]
        ):
            return "TAX_INFO"
            ...
            term in field_lower for term in ["process", "date", "transaction", "cycle"]
        ):
            return "PROCESSING"
        elif any(
            term in field_lower for term in ["business", "entity", "ein", "employer"]
        ):
            return "BUSINESS"
    ...
    def _group_fields_by_section(
        self, field_positions: list[dict[str, Any]]
    ...
    def _create_section_chunk(
        ...
        source_metadata = {
            ...
            "data_type": (
                "MIXED"
                if len(set(f["data_type"] for f in fields)) > 1
                else fields[0]["data_type"] if fields else "UNKNOWN"
            ),
            "fields": fields,
        }
