...
class CreateTablesRequest(BaseModel):
    ...
    force_recreate: bool = Field(False, description="Whether to recreate existing tables")
    tables: List[str] = Field(["all"], description="List of tables to create or 'all'")
    partition_expiration_days: int = Field(365, ge=1, le=7300, description="Partition expiration in days")
...
class CreateIndexRequest(BaseModel):
    ...
    distance_type: str = Field("COSINE", description="Distance type (COSINE, EUCLIDEAN, DOT_PRODUCT)")
...
class CreateIndexesRequest(BaseModel):
    ...
    indexes: List[CreateIndexRequest] = Field(..., description="List of indexes to create")
...
class ValidateSchemaRequest(BaseModel):
    ...
    sample_size: int = Field(1000, ge=100, le=10000, description="Sample size for data validation")
...
async def list_tables(
    include_schema: bool = Query(True, description="Whether to include schema information"),
    schema_manager: SchemaManager = Depends(get_schema_manager)
) -> TablesListResponse:
...
async def delete_tables(
    confirm: bool = Query(False, description="Confirmation required to delete all tables"),
    schema_manager: SchemaManager = Depends(get_schema_manager)
):
    """Delete all tables."""
    if not confirm:
