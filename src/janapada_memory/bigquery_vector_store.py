...
class BigQueryVectorStore:
    ...
    def __init__(
        ...
        self.embeddings_table = f"`{self.project_id}.{self.dataset_id}.source_embeddings`"
        self.metrics_table = f"`{self.project_id}.{self.dataset_id}.gap_metrics`"
        
        logger.info(f"BigQuery vector store initialized: {self.project_id}.{self.dataset_id}")
    ...
    def insert_embedding(
        ...
        try:
            ...
            metadata_errors = self.connection.insert_rows(
                self.connection.client.dataset(self.dataset_id).table("source_metadata"),
                [metadata_record]
            )
            ...
            embedding_errors = self.connection.insert_rows(
                self.connection.client.dataset(self.dataset_id).table("source_embeddings"),
                [embedding_record]
            )
    ...
    def list_embeddings(
        ...
        try:
            ...
            count_job_config = bigquery.QueryJobConfig(
                query_parameters=[p for p in query_params if p.name.startswith("artifact_type_")]
            )
            count_result = list(self.connection.execute_query(count_query, job_config=count_job_config))
    ...
    def search_similar_vectors(
        ...
        try:
            ...
            artifact_filter = ""
            query_params = [
                bigquery.ArrayQueryParameter("query_embedding", "FLOAT64", processed_query),
    ...
    def search_similar_text(
        ...
        logger.warning("search_similar_text using dummy embedding - implement actual text embedding generation")
    ...
    def batch_insert_embeddings(self, embeddings_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for data in embeddings_data:
                ...
                metadata_record = self._prepare_metadata_record(data, data.get("metadata"))
                metadata_records.append(metadata_record)
                
                embedding_record = {
                    "chunk_id": chunk_id,
                    "embedding": embedding,
                    "embedding_model": data.get("embedding_model", self.embedding_model),
                    "created_at": datetime.now(),
                    "partition_date": date.today()
                }
            ...
            if embedding_errors:
                ...
                raise ValueError(f"Failed to batch insert embeddings: {embedding_errors}")
            ...
            for data in embeddings_data:
                results.append({
                    "chunk_id": data["chunk_id"],
                    "status": "inserted",
                    "embedding_dimensions": len(self._process_embedding(data["embedding"])),
                    "timestamp": datetime.now().isoformat()
                })
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["bigquery_connection"] = connection_health["bigquery_connection"]
            ...
            try:
                ...
                health_status["vector_index_status"] = "active" if index_count > 0 else "inactive"
            except Exception as e:
                logger.debug(f"Vector index status check failed: {e}")
                health_status["vector_index_status"] = "unknown"
            
            # Determine overall health
            if (health_status["bigquery_connection"] and 
    ...
    def _process_embedding(self, embedding: List[float]) -> List[float]:
        ...
        if len(embedding) > self.target_dimensions:
            ...
            logger.warning(f"Truncating embedding from {len(embedding)} to {self.target_dimensions} dimensions")
            return embedding[:self.target_dimensions]
        
        # Pad with zeros if too short
        if len(embedding) < self.target_dimensions:
            logger.warning(f"Padding embedding from {len(embedding)} to {self.target_dimensions} dimensions")
