...
class MigrationManager:
    ...
    def start_migration(
        ...
        try:
            ...
            logger.info(
                f"Started migration {migration_id}: {source_type} → {target_type}"
            )
            ...
            return {"status": MigrationStatus.FAILED, "error_message": str(e)}
    ...
    def list_migrations(
        ...
        try:
            ...
            migration_files = [
                f
                for f in os.listdir(self.migration_storage_path)
                if f.endswith(".json")
            ]
        ...
        try:
            ...
            report = {
                ...
                "data_integrity": {
                    ...
                    "successful_migrations": metadata["progress"]["processed_records"]
                    - metadata["progress"]["failed_records"],
                    "failed_migrations": metadata["progress"]["failed_records"],
                    "success_rate": self._calculate_success_rate(metadata["progress"]),
                },
                "similarity_preservation": metadata["quality_metrics"].get(
                    "similarity_preservation", {}
                ),
                "performance_metrics": metadata["quality_metrics"].get(
                    "performance", {}
                ),
                "quality_assessment": {
                    "overall_score": metadata["quality_metrics"].get(
                        "overall_quality_score", 0.0
                    ),
                    "dimension_reduction_quality": metadata["quality_metrics"].get(
                        "dimension_reduction", {}
                    ),
                    "data_integrity_score": self._calculate_success_rate(
                        metadata["progress"]
                    ),
                },
            ...
            logger.error(
                f"Failed to generate validation report for {migration_id}: {e}"
            )
            return None
    ...
    def get_quality_report(self, migration_id: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "similarity_preservation_score": quality_metrics.get(
                    "similarity_correlation", 0.0
                ),
                "performance_score": min(
                    1.0, quality_metrics.get("throughput", 0) / 100
                ),  # Normalize
    ...
    def test_dimension_reduction(
        ...
        try:
            ...
            reduced_embeddings, training_stats = self.dimension_reducer.fit_transform(
                embeddings
            )

            # Evaluate quality
            quality_metrics = self.dimension_reducer.evaluate_quality(
    ...
    def get_migration_performance(self, migration_id: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "error_rate": progress["failed_records"]
                / max(1, progress["processed_records"]),
    ...
    def _save_migration_metadata(
        self, migration_id: str, metadata: dict[str, Any]
    ) -> None:
    ...
    def _generate_migration_recommendations(
        ...
        if not recommendations:
            recommendations.append(
                "Migration completed successfully with good quality metrics"
            )
