...
class BigQueryConnection:
    ...
    def __init__(
        ...
        self.dataset_id = dataset_id or os.getenv("BIGQUERY_DATASET_ID", "semantic_gap_detector")
        ...
        if not self.project_id:
            raise ValueError("Google Cloud project ID must be provided via parameter or GOOGLE_CLOUD_PROJECT env var")
        
        logger.info(f"Initializing BigQuery connection to project: {self.project_id}, dataset: {self.dataset_id}")
    ...
    def dataset_ref(self) -> bigquery.DatasetReference:
        ...
        if self._dataset_ref is None:
            self._dataset_ref = bigquery.DatasetReference(self.project_id, self.dataset_id)
    ...
    def _validate_connection(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Dataset {self.dataset_id} does not exist, will be created during setup")
    ...
    def execute_query(
        ...
        for attempt in range(self.max_retries):
            try:
                logger.debug(f"Executing query (attempt {attempt + 1}/{self.max_retries}): {query[:100]}...")
                
                query_job = self.client.query(query, job_config=job_config)
                results = query_job.result(timeout=timeout)
                
                logger.debug(f"Query completed successfully. Rows processed: {query_job.total_bytes_processed}")
                return results
                ...
                if attempt == self.max_retries - 1:
                    logger.error(f"Query failed after {self.max_retries} attempts: {query[:100]}...")
                    raise
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if errors:
                    ...
                    logger.debug(f"Successfully inserted {len(rows)} rows into {table_ref.table_id}")
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if health_status["bigquery_connection"] and health_status["dataset_accessible"]:
