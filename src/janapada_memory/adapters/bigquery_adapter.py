...
class BigQueryAdapter:
    ...
    def search_similar_vectors(
        ...
        with self.connection.query_context("vector_similarity_search") as context:
            try:
                ...
                results = list(
                    self.connection.execute_query(query, job_config=job_config)
                )
    ...
    def validate_vector_index(
        self, table_name: str = "source_embeddings"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            index_results = list(
                self.connection.execute_query(index_query, job_config=job_config)
            )
