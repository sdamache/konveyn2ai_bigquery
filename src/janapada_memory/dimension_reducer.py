...
class DimensionReducer:
    ...
    def __init__(
        ...
        logger.info(
            f"DimensionReducer initialized: {self.target_dimensions} dimensions, {self.preserve_variance:.1%} variance"
        )

    def fit(
        self, embeddings: Union[list[list[float]], np.ndarray], save_model: bool = True
    ) -> dict[str, Any]:
        """
        ...
        components_for_variance = (
            np.searchsorted(cumsum_variance, self.preserve_variance) + 1
        )
        ...
        logger.info(
            f"Components needed for {self.preserve_variance:.1%} variance: {components_for_variance}"
        )
        ...
        logger.info(
            f"Compression ratio: {self.training_stats['compression_ratio']:.1f}x"
        )
    ...
    def transform(self, embeddings: Union[list[list[float]], np.ndarray]) -> np.ndarray:
        """
    ...
    def inverse_transform(
        self, reduced_embeddings: Union[list[list[float]], np.ndarray]
    ) -> np.ndarray:
        """
    ...
    def evaluate_quality(
        ...
        reduced_flat = reduced_similarities[
            np.triu_indices_from(reduced_similarities, k=1)
        ]
        ...
        quality_metrics = {
            "variance_explained": (
                float(self.variance_explained) if self.variance_explained else 0.0
            ),
        ...
        logger.info(
            f"  Variance explained: {quality_metrics['variance_explained']:.1%}"
        )
        logger.info(
            f"  Similarity correlation: {quality_metrics['similarity_correlation']:.3f}"
        )
        logger.info(
            f"  Neighbor preservation: {quality_metrics['neighbor_preservation']:.1%}"
        )
    ...
    def _evaluate_neighbor_preservation(
        ...
        for i in range(len(original_embeddings)):
            ...
            orig_similarities = cosine_similarity(
                [original_embeddings[i]], original_embeddings
            )[0]
            orig_neighbors = np.argsort(orig_similarities)[::-1][
                1 : k + 1
            ]  # Exclude self

            # Find k nearest neighbors in reduced space
            reduced_similarities = cosine_similarity(
                [reduced_embeddings[i]], reduced_embeddings
            )[0]
            reduced_neighbors = np.argsort(reduced_similarities)[::-1][
                1 : k + 1
            ]  # Exclude self
    ...
    def load_model(self, path: Optional[str] = None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Model dimensions: {self.training_stats.get('original_dimensions')} → {self.training_stats.get('actual_dimensions')}"
            )
    ...
    def batch_transform(
        self, embeddings_list: list[list[float]], batch_size: int = 1000
    ) -> list[list[float]]:
        """
        ...
        logger.info(
            f"Batch transforming {len(embeddings_list)} embeddings in batches of {batch_size}"
        )
        ...
        for i in range(0, len(embeddings_list), batch_size):
            ...
            if (i // batch_size + 1) % 10 == 0:
                logger.info(
                    f"Processed {i + len(batch)}/{len(embeddings_list)} embeddings"
                )

        logger.info(
            f"Batch transformation completed: {len(reduced_embeddings)} embeddings"
        )
        return reduced_embeddings
