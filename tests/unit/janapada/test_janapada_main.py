...
class TestJanapadaMemory:
    ...
    @pytest.fixture
    def client(self, janapada_app):
        """Test client for Janapada service."""
        return TestClient(janapada_app)
    ...
    async def test_search_jsonrpc_success(
        self, client, mock_vertex_setup, mock_matching_engine, sample_snippets
    ):
    ...
    async def test_search_with_embedding_failure(
        self, client, mock_env_vars, mock_google_credentials
    ):
        ...
        with patch(
            ...
            model_instance.get_embeddings.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_search_with_index_failure(self, client, mock_vertex_setup):
    ...
    async def test_search_result_metadata_parsing(
        self, client, mock_vertex_setup, sample_snippets
    ):
        ...
        with patch(
            ...
            incomplete_match.metadata = json.dumps(
                {"file_path": "test.py"}
            )  # Missing content
        ...
        def make_request(request_id):
            try:
                jsonrpc_request = {
                    ...
                    "method": "search",
                    "params": {"query": f"test query {request_id}", "k": 3},
                }
...
class TestJanapadaConfiguration:
    ...
    def test_environment_variable_validation(self):
        ...
        required_vars = ["GOOGLE_CLOUD_PROJECT", "INDEX_ENDPOINT_ID", "INDEX_ID"]
    ...
    def test_search_performance(self, client, mock_vertex_setup, mock_matching_engine):
        ...
        for i in range(50):
            jsonrpc_request = {
                ...
                "id": f"perf-test-{i}",
                "method": "search",
                "params": {"query": f"performance test query {i}", "k": 5},
            }
