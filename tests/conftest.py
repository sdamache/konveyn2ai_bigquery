...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(fixture in item.fixturenames for fixture in ["docker_compose", "docker_client"]):
            item.add_marker(pytest.mark.docker)
        
        # Mark tests that use external service fixtures  
        if any(fixture in item.fixturenames for fixture in ["mock_vertex_ai", "mock_gemini_ai"]):
