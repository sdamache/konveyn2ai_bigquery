...
class TestSetupWorkflow:
    ...
    def test_make_setup_command_success(self):
        ...
        print("✅ make setup completed successfully")
    ...
    def test_dataset_creation(self, schema_manager):
        ...
        print(
            f"✅ Dataset created: {dataset_info['project_id']}.{dataset_info['dataset_id']}"
        )
    ...
    def test_vector_index_creation(self, schema_manager):
        ...
        index_info = next(
            idx
            for idx in indexes_info["indexes"]
        ...
        while wait_time < max_wait_time:
            current_status = schema_manager.get_index_status(
            ...
            if current_status["status"] == "ACTIVE":
                ...
                pytest.fail(
                    f"Vector index creation failed: {current_status.get('error_message', 'Unknown error')}"
                )
        ...
        assert final_status["status"] in [
            "ACTIVE",
            "CREATING",
        ], f"Index status: {final_status['status']}"
    ...
    def test_setup_idempotency(self):
        ...
        result1 = subprocess.run(
            ["make", "setup"], capture_output=True, text=True, env=test_env, timeout=60
        )
    ...
    def test_make_diagnose_after_setup(self):
        ...
        assert any(
            keyword in output
            for keyword in ["bigquery", "connection", "status", "tables"]
        )
    ...
    def test_setup_completion_indicators(self):
        ...
        found_indicators = [
            indicator for indicator in success_indicators if indicator in output.lower()
        ]
        assert (
            len(found_indicators) > 0
        ), f"No success indicators found in output: {output}"
            ...
            if os.path.exists(log_file):
                ...
                with open(log_file) as f:
                    ...
                    assert any(
                        ...
                        for keyword in ["setup", "create", "bigquery"]
                    )
