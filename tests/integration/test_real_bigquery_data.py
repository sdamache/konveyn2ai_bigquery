...
class RealBigQueryDataTest:
    ...
    def generate_realistic_sample_data(self) -> List[Dict[str, Any]]:
        ...
        def create_similar_embedding(
            base_seed: int, similarity_factor: float = 0.8
        ) -> List[float]:
...
- Cosine similarity search with BigQuery VECTOR_SEARCH""",
                ...
                "metadata": {"version": "v1.2", "endpoints": 1, "examples": True},
            },
            {
...
def manhattan_distance(point_a, point_b):
    ...
    "performance": {
        ...
        "cache_ttl": "1h"
                ...
                "metadata": {"format": "json", "sections": 3, "validated": True},
            },
        ]
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"⚠️ Component initialization failed (expected without credentials): {init_e}"
                    )
    ...
    def test_schema_setup(self):
        ...
        try:
            ...
            assert validation.get(
                "valid", False
            ), f"Schema validation failed: {validation}"
    ...
    def test_sample_data_quality(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            for i, item in enumerate(sample_data):
                ...
                assert (
                    len(embedding) == 768
                ), f"Item {i} embedding wrong dimensions: {len(embedding)}"
                assert all(
                    isinstance(x, (int, float)) for x in embedding
                ), f"Item {i} embedding contains non-numeric values"

                # Test semantic patterns (vectors should have non-zero values)
                embedding_array = np.array(embedding)
                assert (
                    np.linalg.norm(embedding_array) > 0
                ), f"Item {i} embedding is zero vector"

                # Test text content quality
                assert (
                    len(item["text_content"]) > 20
                ), f"Item {i} text content too short"

                # Test metadata structure
                if "metadata" in item:
                    assert isinstance(
                        item["metadata"], dict
                    ), f"Item {i} metadata not a dict"
            ...
            for i in range(len(embeddings)):
                for j in range(i + 1, len(embeddings)):
                    ...
                    assert (
                        similarity < 0.99
                    ), f"Embeddings {i} and {j} too similar: {similarity:.4f}"

            logger.info(
                f"✅ Sample data quality validated: {len(sample_data)} items with 768-dim embeddings"
            )
            self.results["passed_tests"] += 1
        ...
        try:
            ...
            for item in sample_data:
                try:
                    ...
                    assert (
                        result is not None
                    ), f"Insertion failed for {item['chunk_id']}"
            ...
            if successful_insertions > 0:
                logger.info(
                    f"✅ Inserted {successful_insertions}/{len(sample_data)} vectors in {insertion_time:.2f}s"
                )
                self.results["passed_tests"] += 1
            else:
                logger.info(
                    "⚠️ No successful insertions - test infrastructure may need credentials"
                )
    ...
    def test_vector_retrieval_accuracy(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            for item in sample_data:
                try:
                    ...
                    if retrieved is not None:
                        ...
                        assert (
                            retrieved["text_content"] == item["text_content"]
                        ), "Content mismatch"
                        assert retrieved["source"] == item["source"], "Source mismatch"
                        assert (
                            retrieved["artifact_type"] == item["artifact_type"]
                        ), "Type mismatch"
                        assert (
                            len(retrieved["embedding"]) == 768
            ...
            logger.info(
                f"✅ Retrieved {successful_retrievals}/{len(sample_data)} vectors in {retrieval_time:.2f}s"
            )
            self.results["passed_tests"] += 1
    ...
    def test_similarity_search_quality(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.info(
                f"✅ Similarity search returned {len(results)} results in {search_time:.2f}s"
            )
            self.results["passed_tests"] += 1
    ...
    def test_filtering_and_metadata_search(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            code_results = self.vector_store.list_embeddings(
                limit=10, artifact_types=["code"], include_embeddings=False
            )
            ...
            for embedding in code_results["embeddings"]:
                assert (
                    embedding["artifact_type"] == "code"
                ), f"Non-code result: {embedding['artifact_type']}"

            # Test documentation filtering
            doc_results = self.vector_store.list_embeddings(
                limit=10, artifact_types=["documentation"], include_embeddings=False
            )

            if doc_results and doc_results["embeddings"]:
                for embedding in doc_results["embeddings"]:
                    assert (
                        embedding["artifact_type"] == "documentation"
    ...
    def test_batch_operations_performance(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            assert len(batch_results) == len(
                batch_data
            ), "Batch insertion count mismatch"
            ...
            assert (
                len(retrieved_batch) >= len(chunk_ids) * 0.8
            ), "Too many batch retrieval failures"
            ...
            logger.info(
                f"✅ Batch operations: insert {batch_insert_time:.2f}s, retrieve {batch_retrieve_time:.2f}s"
            )
            self.results["passed_tests"] += 1
    ...
    def test_concurrent_access_handling(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            retrieve_result = self.vector_store.get_embedding_by_id(
                test_chunk["chunk_id"]
            )
            assert retrieve_result is not None, "Concurrent retrieve failed"
            assert (
                retrieve_result["chunk_id"] == test_chunk["chunk_id"]
            ), "Consistency error"
    ...
    def test_real_time_query_performance(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            assert (
                avg_query_time < performance_threshold
            ), f"Average query time too slow: {avg_query_time:.1f}ms > {performance_threshold}ms"

            logger.info(
                f"✅ Query performance: avg {avg_query_time:.1f}ms, max {max_query_time:.1f}ms"
            )
            self.results["passed_tests"] += 1
    ...
    def test_round_trip_data_integrity(self, sample_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            # Insert
            insert_result = self.vector_store.insert_embedding(
                chunk_data=test_item, embedding=test_item["embedding"]
            )
            ...
            assert (
                retrieved["text_content"] == test_item["text_content"]
            ), "Content corrupted"
            assert retrieved["source"] == test_item["source"], "Source corrupted"
            assert (
                retrieved["artifact_type"] == test_item["artifact_type"]
            ), "Type corrupted"
            ...
            assert (
                cosine_sim > 0.99
            ), f"Embedding corrupted: similarity = {cosine_sim:.4f}"
    ...
    def test_error_handling_with_real_scenarios(self):
        ...
        try:
            ...
            try:
                result = self.vector_store.get_embedding_by_id(
                    "non_existent_chunk_12345"
                )
            ...
            try:
                ...
                result = self.vector_store.insert_embedding(
                    chunk_data=invalid_chunk, embedding=invalid_chunk["embedding"]
                )
    ...
    def test_system_health_monitoring(self):
        ...
        try:
            ...
            logger.info(
                f"Connection: {health_status.get('bigquery_connection', 'unknown')}"
            )
            logger.info(
                f"Embeddings: {health_status.get('embedding_count', 'unknown')}"
            )
