...
def test_source_metadata_errors_required_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_fields.items():
        assert field_name in actual_fields, f"Required field '{field_name}' missing from error schema"
...
def test_source_metadata_errors_field_modes(bigquery_client, dataset_id):
    ...
    required_fields = {
        'error_id', 'source_type', 'source_uri', 'error_class',
        'error_msg', 'collected_at', 'tool_version'
    }

    # Optional fields (can be NULL)
    nullable_fields = {
        'sample_text', 'stack_trace'
    }
    ...
    for field_name in required_fields:
        assert field_name in field_modes, f"Required field '{field_name}' missing from error schema"
...
@pytest.mark.contract
@pytest.mark.bigquery
def test_source_metadata_errors_partitioning_and_clustering(bigquery_client, dataset_id):
...
def test_source_metadata_errors_constraints(bigquery_client, dataset_id):
    ...
    for field_name in essential_fields:
        assert field_name in field_names, f"Essential error field '{field_name}' missing"

    # Verify error_class field can handle expected error types
    error_class_field = next(field for field in table.schema if field.name == 'error_class')
    assert error_class_field.field_type == 'STRING', "error_class should be STRING type"

    # Verify source_type field matches main table
    source_type_field = next(field for field in table.schema if field.name == 'source_type')
...
def test_source_metadata_errors_field_descriptions(bigquery_client, dataset_id):
    ...
    key_fields_requiring_description = {
        'error_id', 'error_class', 'error_msg', 'sample_text'
    }
