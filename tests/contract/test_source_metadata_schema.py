...
def test_source_metadata_required_common_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_fields.items():
        assert field_name in actual_fields, f"Required field '{field_name}' missing from schema"
...
def test_source_metadata_kubernetes_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_k8s_fields.items():
        assert field_name in actual_fields, f"Kubernetes field '{field_name}' missing from schema"
        assert actual_fields[field_name] == expected_type, \
            f"Field '{field_name}' has type '{actual_fields[field_name]}', expected '{expected_type}'"
...
def test_source_metadata_fastapi_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_fastapi_fields.items():
        assert field_name in actual_fields, f"FastAPI field '{field_name}' missing from schema"
        assert actual_fields[field_name] == expected_type, \
            f"Field '{field_name}' has type '{actual_fields[field_name]}', expected '{expected_type}'"
...
def test_source_metadata_cobol_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_cobol_fields.items():
        assert field_name in actual_fields, f"COBOL field '{field_name}' missing from schema"
        assert actual_fields[field_name] == expected_type, \
            f"Field '{field_name}' has type '{actual_fields[field_name]}', expected '{expected_type}'"
...
def test_source_metadata_irs_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_irs_fields.items():
        assert field_name in actual_fields, f"IRS field '{field_name}' missing from schema"
        assert actual_fields[field_name] == expected_type, \
            f"Field '{field_name}' has type '{actual_fields[field_name]}', expected '{expected_type}'"
...
def test_source_metadata_mumps_fields(bigquery_client, dataset_id):
    ...
    for field_name, expected_type in expected_mumps_fields.items():
        assert field_name in actual_fields, f"MUMPS field '{field_name}' missing from schema"
        assert actual_fields[field_name] == expected_type, \
            f"Field '{field_name}' has type '{actual_fields[field_name]}', expected '{expected_type}'"
...
def test_source_metadata_partitioning_and_clustering(bigquery_client, dataset_id):
    ...
    assert table.time_partitioning.field == "collected_at", \
...
def test_source_metadata_field_modes(bigquery_client, dataset_id):
    ...
    required_fields = {
        ...
        'created_at', 'updated_at', 'collected_at', 'source_uri', 'tool_version'
    }

    # All source-specific fields should be NULLABLE
    nullable_fields = {
        'parent_id', 'parent_type', 'content_tokens', 'repo_ref',
        # Kubernetes fields
        'k8s_api_version', 'k8s_kind', 'k8s_namespace', 'k8s_resource_name',
        'k8s_labels', 'k8s_annotations', 'k8s_resource_version',
        # FastAPI fields
        'fastapi_http_method', 'fastapi_route_path', 'fastapi_operation_id',
        'fastapi_request_model', 'fastapi_response_model', 'fastapi_status_codes',
        'fastapi_dependencies', 'fastapi_src_path', 'fastapi_start_line', 'fastapi_end_line',
        # COBOL fields
        'cobol_structure_level', 'cobol_field_names', 'cobol_pic_clauses',
        'cobol_occurs_count', 'cobol_redefines', 'cobol_usage',
        # IRS fields
        'irs_record_type', 'irs_layout_version', 'irs_start_position',
        'irs_field_length', 'irs_data_type', 'irs_section',
        # MUMPS fields
        'mumps_global_name', 'mumps_node_path', 'mumps_file_no',
        'mumps_field_no', 'mumps_xrefs', 'mumps_input_transform'
    }
