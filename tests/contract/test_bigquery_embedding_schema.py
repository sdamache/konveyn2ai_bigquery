...
class TestBigQueryEmbeddingSchema:
    ...
    def test_embedding_field_is_repeated_array(self, bigquery_client, dataset_id):
        ...
        assert (
            embedding_field.field_type == "FLOAT64"
        ), "embedding field should be FLOAT64"
        assert (
            embedding_field.mode == "REPEATED"
        ), "embedding field should be REPEATED (array)"
    ...
    def test_composite_primary_key_constraint(self, bigquery_client, dataset_id):
        ...
        assert (
            "chunk_id" in description
        ), "Table description should mention chunk_id in primary key"
        assert (
            "model" in description
        ), "Table description should mention model in primary key"
        assert (
            "content_hash" in description
        ), "Table description should mention content_hash in primary key"
    ...
    def test_vector_search_functions_available(self, bigquery_client, dataset_id):
        ...
        try:
            ...
            assert (
                query_job is not None
            ), "Should be able to query source_embeddings table"
        except NotFound:
...
def test_vector_search_prerequisites():
    ...
    ), "BIGQUERY_DATASET_ID environment variable must be set"
