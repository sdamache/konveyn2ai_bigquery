...
class TestEmbeddingCacheContract:
    ...
    def test_embedding_cache_constructor(self, temp_cache_dir):
        ...
        try:
            ...
            assert any(
                ...
                for keyword in ["not implemented", "notimplementederror", "attribute"]
            ), f"Expected TDD failure, got: {e}"

    def test_cache_key_generation_method(
        self, temp_cache_dir, sample_content, embedding_model
    ):
        ...
        try:
            ...
            assert (
                len(cache_key) > 32
            ), "Cache key should include content hash (64 chars + model)"
    ...
    def test_cache_get_method_interface(
        self, temp_cache_dir, sample_content, embedding_model
    ):
    ...
    def test_cache_set_method_interface(
        self, temp_cache_dir, sample_content, embedding_model, sample_embedding
    ):
    ...
    def test_cache_persistence_across_instances(
        ...
        try:
            ...
            assert (
                retrieved == sample_embedding
            ), "Retrieved embedding should match stored embedding"
    ...
    def test_cache_file_format_json(
        ...
        try:
            ...
            assert isinstance(
                cache_data["vector"], list
            ), "Cached vector should be list"
            assert (
                len(cache_data["vector"]) == 768
            ), "Cached vector should be 768 dimensions"
            assert (
                cache_data["vector"] == sample_embedding
            ), "Cached vector should match"
            assert isinstance(
                cache_data["created_at"], str
            ), "Created timestamp should be string"
            assert isinstance(
                cache_data["content_hash"], str
            ), "Content hash should be string"
            assert (
                len(cache_data["content_hash"]) == 64
            ), "Content hash should be SHA256 (64 chars)"
    ...
    def test_cache_content_hash_collision_handling(
        self, temp_cache_dir, embedding_model, sample_embedding
    ):
        ...
        try:
            ...
            assert (
                result1 != result2
            ), "Different content should have different cached embeddings"
    ...
    def test_cache_model_separation(
        self, temp_cache_dir, sample_content, sample_embedding
    ):
        ...
        try:
            ...
            assert (
                result1 != result2
            ), "Different models should have different embeddings"
...
def test_cache_directory_structure():
    ...
    assert os.path.isdir(
        cache_dir
    ), f"Cache directory {cache_dir} should be a directory"

    # Should be writable
    assert os.access(
        cache_dir, os.W_OK
    ), f"Cache directory {cache_dir} should be writable"
