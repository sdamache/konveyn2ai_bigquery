...
def analyze_contract_test_results():
    ...
    for test_name, result in contract_results["vector_search_contract"][
        "details"
    ].items():
    ...
    try:
        ...
        results = memory_service.similarity_search(query_embedding=test_embedding, k=1)
    ...
    logger.info(
        "1. Update contract tests to use 'connection.client' instead of '_bigquery_client'"
    )
