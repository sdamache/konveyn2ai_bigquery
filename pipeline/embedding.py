...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        logger.info(
            f"Embedding generator initialized with model {model}, batch_size {batch_size}"
        )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response and "embedding" in response and response["embedding"]:
                    ...
                    logger.warning(
                        f"Empty embedding response for content: {content[:100]}..."
                    )
...
class EmbeddingPipeline:
    ...
    def _ensure_tables_exist(self) -> None:
        ...
        try:
            ...
            result = self.schema_manager.create_tables(
                tables=["source_metadata", "source_embeddings"], force_recreate=False
            )
    ...
    def _get_pending_chunks(
        self, limit: Optional[int] = None, where_clause: str = ""
    ) -> List[Dict[str, Any]]:
    ...
    def generate_embeddings(
        self, limit: Optional[int] = None, where_clause: str = "", dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        # Get pending chunks
        pending_chunks = self._get_pending_chunks(
            limit=limit, where_clause=where_clause
        )
        ...
        for i in range(0, len(pending_chunks), self.generator.batch_size):
            ...
            total_batches = (
                len(pending_chunks) + self.generator.batch_size - 1
            ...
            if not dry_run:
                ...
                logger.info(
                    f"DRY RUN: Would store {len([e for e in batch_embeddings if e['embedding']])} embeddings"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate embeddings for BigQuery vector backend"
    )
    ...
    parser.add_argument(
        "--where", default="", help="Additional WHERE clause for filtering"
    )
    parser.add_argument(
        "--cache-dir", default=".cache/embeddings", help="Cache directory"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't actually store embeddings"
    )
    ...
    try:
        ...
        result = pipeline.generate_embeddings(
            limit=args.limit, where_clause=args.where, dry_run=args.dry_run
        )
