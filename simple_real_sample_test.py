    ...
    for file_path in sample_files:
        ...
        if full_path.exists():
            try:
                ...
                artifact_type = "kubernetes" if "k8s-manifests" in file_path else "cloud-run"
...
def test_vector_search_with_samples():
    ...
    try:
            ...
            try:
                ...
                for result in vector_results:
                    processed_results.append({
                        ...
                        "text_preview": result.text_content[:150] + "..." if len(result.text_content) > 150 else result.text_content
                    })
                ...
                print(f"    ✅ Found {len(vector_results)} results in {search_time*1000:.1f}ms")
        ...
        print("✅ BigQuery Memory Adapter successfully demonstrated with repository sample files")
        
        # Print summary
        print("\n📋 DEMO SUMMARY:")
        print(f"  • Analyzed {len(samples)} real repository files")
        print(f"  • Performed {len(search_results)} semantic searches")
        print(f"  • Average search time: {sum(sr.get('search_time_ms', 0) for sr in search_results)/len(search_results):.1f}ms")
        print(f"  • Total results found: {sum(sr.get('results_count', 0) for sr in search_results)}")
