...
def main():
    ...
    if not sys.prefix != sys.base_prefix:
        ...
        print("   Or run: python -m venv venv && source venv/bin/activate && pip install -r requirements.txt")
        sys.exit(1)
    ...
    while True:
        ...
        if choice == "0":
            ...
            success, _ = run_command(
                ["pytest", "tests/", "-v", "-m", "not docker", "--cov=src", "--cov-report=html", "--cov-report=term-missing"],
            ...
            success, _ = run_command(
                ["pytest", "tests/unit/common/test_models.py", "tests/unit/svami/test_main.py::TestSvamiOrchestrator::test_health_endpoint", "-v"],
