    ...
    def test_vertex_ai_components(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "embedding_dimensions": len(embeddings[0].values)
                if embeddings
                else None,
                "vector_index_name": vector_index.display_name,
                "vector_index_id": "805460437066842112",
                "test_embedding_preview": embeddings[0].values[:5]
                if embeddings
                else None,
            }

            logger.info(
                f"✅ Vertex AI components working: {result['embedding_dimensions']} dims"
            )
            return result
    ...
    def start_service_with_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "process_id": self.service_process.pid
                if self.service_process.poll() is None
                else None,
            }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result["subtask_6_3_validation"] = {
                    "embedding_model_ready": components.get("embedding_model")
                    == "ready",
                    "matching_engine_ready": components.get("matching_engine")
                    == "ready",
                    "vector_search_enabled": features.get("vector_search") == True,
                    "correct_dimensions": components.get("embedding_dimensions") == 768,
                    "correct_model": components.get("embedding_model_name")
                    == "text-embedding-004",
                }
        ...
        try:
            ...
            for query in test_queries:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"✅ Debug test for '{query[:30]}...': {result['embedding_dimensions']} dims"
                    )
                else:
                    results.append(
                        {
            ...
            return {
                ...
                "successful_tests": len(
                    [r for r in results if r.get("embedding_available")]
                ),
            }
        ...
        try:
            ...
            for test_case in test_queries:
                rpc_request = {
                    "jsonrpc": "2.0",
                    "method": "search",
                    "params": {"query": test_case["query"], "k": test_case["k"]},
                    ...
                    if "result" in rpc_response:
                        ...
                        for snippet in snippets:
                            ...
                            if (
                                ...
                                or "Vector search result" in content
                            ):
                        ...
                        results.append(result)
                        logger.info(
                            f"✅ Vector search for '{test_case['query']}': {len(snippets)} results"
                        )
                    else:
                        results.append(
                            {
            ...
            return {
                ...
                "total_tests": len(test_queries),
                "successful_tests": len(
                    [r for r in results if r.get("snippets_returned", 0) > 0]
                ),
                "vector_search_working": any(
                    r.get("has_vector_search_results", False) for r in results
                ),
            }
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.setup_environment():
                test_results["tests"]["environment_setup"] = {
                    "status": "failed",
            ...
            ] = self.test_vertex_ai_components()
            ...
            test_results["tests"][
                "health_endpoint"
            ] = self.test_health_endpoint_comprehensive()
            ...
            test_results["tests"][
                "vector_search"
            ] = self.test_vector_search_comprehensive()
