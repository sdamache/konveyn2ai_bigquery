        ...
        try:
            ...
            if isinstance(data, dict):
                analysis.update({
                    ...
                    "metadata_name": data.get("metadata", {}).get("name", "unknown")
                })
                
                # Kubernetes-specific analysis
                if "apps/v1" in analysis["api_version"] or analysis["kind"] in ["Deployment", "Service"]:
                    ...
                    if analysis["kind"] == "Deployment":
                        ...
                        analysis.update({
                            ...
                            "has_resources": bool(spec.get("template", {}).get("spec", {}).get("containers", [{}])[0].get("resources")),
                            "has_probes": any("Probe" in str(c) for c in spec.get("template", {}).get("spec", {}).get("containers", []))
                        })
                ...
                # Cloud Run-specific analysis
                    ...
                    analysis.update({
                        ...
                        "container_concurrency": spec.get("containerConcurrency", 0),
...
def extract_semantic_features(content: str) -> Dict[str, Any]:
    ...
    # Technical terms
    tech_keywords = [
        "nginx", "kubernetes", "docker", "container", "deployment", "service",
        "configmap", "pod", "replica", "ingress", "cloud run", "autoscaling",
        "cpu", "memory", "port", "environment", "health", "liveness", "readiness"
    ]
    ...
    config_patterns = [
        "resource limits", "health checks", "environment variables",
        "port configuration", "service discovery", "load balancing",
    ...
    if "resources:" in content and ("cpu" in content_lower or "memory" in content_lower):
...
def simulate_vector_search(query: str, file_analyses: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for analysis in file_analyses:
        ...
        if similarity > 0.1:  # Only include reasonably similar results
            results.append({
                ...
                "relevance_factors": analysis.get("semantic_features", {}).get("technical_terms", [])
            })
...
def generate_hackathon_demo():
    ...
    for file_path in sample_files:
        ...
        if full_path.exists():
            try:
                ...
                analysis["sample_content"] = content[:500] + "..." if len(content) > 500 else content
                
                file_analyses.append(analysis)
                print(f"  ✅ {file_path}: {analysis['artifact_type']} ({analysis['size_bytes']} bytes)")
    ...
    search_scenarios = [
        {
            ...
            "expected_matches": ["kubernetes deployment with nginx", "resource management", "health monitoring"]
        },
        {
            "name": "Service Configuration Search",
            "query": "service port configuration and load balancing setup",
            "expected_matches": ["kubernetes service", "port configuration", "load balancing"]
        },
        {
            "name": "Cloud Native Scaling Search",
            "query": "container orchestration with autoscaling and resource management",
            "expected_matches": ["cloud run service", "autoscaling", "resource management"]
        },
        {
            "name": "Docker Composition Search",
            "query": "docker container configuration with environment variables",
            "expected_matches": ["docker-compose", "environment variables", "container configuration"]
        }
    ]
    ...
    for scenario in search_scenarios:
        ...
        search_result = {
            ...
            "top_matches": results,
            "search_quality": "high" if len(results) >= 2 else "medium" if len(results) == 1 else "low"
        }
    ...
    metrics = {
        "data_ingestion": {
            ...
            "artifact_types_supported": list(set(a["artifact_type"] for a in file_analyses)),
            "metadata_fields_extracted": ["kind", "api_version", "replicas", "service_type", "resource_limits"]
        },
        "semantic_analysis": {
            "technical_terms_identified": list(set(
                term for analysis in file_analyses 
                for term in analysis.get("semantic_features", {}).get("technical_terms", [])
            )),
            "configuration_concepts": list(set(
                concept for analysis in file_analyses 
                for concept in analysis.get("semantic_features", {}).get("configuration_concepts", [])
            )),
            "infrastructure_patterns": list(set(
                pattern for analysis in file_analyses 
                for pattern in analysis.get("semantic_features", {}).get("infrastructure_patterns", [])
            ))
        },
        "search_capabilities": {
            "search_scenarios_tested": len(search_scenarios),
            "average_results_per_query": sum(len(sr["top_matches"]) for sr in search_results) / len(search_results),
            "high_quality_searches": len([sr for sr in search_results if sr["search_quality"] == "high"]),
