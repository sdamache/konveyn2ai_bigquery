    ...
    def test_vertex_ai_embeddings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if embeddings and len(embeddings) > 0:
                ...
                logger.info(f"✅ Vertex AI embeddings working: {len(embedding_vector)} dimensions")
                return result
            else:
    ...
    def test_debug_embedding_endpoint(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Debug embedding test passed: {debug_data.get('embedding_dimensions')} dims")
                return result
            else:
                raise Exception(f"HTTP {response.status_code}: {response.text}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "result" in rpc_response:
                    ...
                    logger.info(f"✅ JSON-RPC search successful: {len(snippets)} snippets returned")
                    return result
                else:
                    raise Exception(f"RPC Error: {rpc_response.get('error', 'Unknown error')}")
            else:
                raise Exception(f"HTTP {response.status_code}: {response.text}")
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.setup_credentials():
                test_results["tests"]["credentials_setup"] = {"status": "failed", "error": "Failed to setup credentials"}
                return test_results
            ...
            test_results["tests"]["vertex_ai_embeddings"] = self.test_vertex_ai_embeddings()
            
            # Test 4: Start service
            if not self.start_janapada_service():
                test_results["tests"]["service_startup"] = {"status": "failed", "error": "Failed to start service"}
                return test_results
