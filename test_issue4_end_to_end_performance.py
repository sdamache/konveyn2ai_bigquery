...
def test_issue4_end_to_end_performance():
    ...
    try:
        ...
        performance_results["document_loading"] = {
            ...
            "total_content_size_bytes": sum(
                doc["metadata"]["size_bytes"] for doc in documents
            ),
        }

        logger.info(
            f"✅ Loaded {len(documents)} documents in {performance_results['document_loading']['loading_time_ms']:.1f}ms"
        )
        logger.info(
            f"📊 Artifact types: {performance_results['document_loading']['artifact_types']}"
        )
        logger.info(
            f"📏 Total content: {performance_results['document_loading']['total_content_size_bytes']} bytes"
        )
        ...
        logger.info(
            f"🔗 BigQuery connection: {'✅ Healthy' if health['bigquery_connection']['is_healthy'] else '❌ Unhealthy'}"
        )
        ...
        logger.info(
            f"⏱️  Total ingestion time: {ingestion_metrics['total_ingestion_time_ms']:.1f}ms"
        )
        logger.info(
            f"🧠 Average embedding time: {ingestion_metrics['total_embedding_time_ms'] / max(1, ingestion_metrics['embeddings_generated']):.1f}ms per document"
        )
        ...
        test_queries = [
            {
                "text": "kubernetes deployment nginx container",
                "types": ["kubernetes"],
                "description": "K8s deployment search",
            },
            {
                "text": "fastapi user model endpoint",
                "types": ["fastapi"],
                "description": "FastAPI code search",
            },
            {
                "text": "customer record name address",
                "types": ["cobol"],
                "description": "COBOL structure search",
            },
            {
                "text": "social security number taxpayer",
                "types": ["irs"],
                "description": "IRS field search",
            },
            {
                "text": "patient name date birth",
                "types": ["mumps"],
                "description": "MUMPS field search",
            },
            {
                "text": "api web application service",
                "types": None,
                "description": "Cross-artifact search",
            },
        ]
        ...
        for query in test_queries:
            ...
            try:
                ...
                results = memory_service.similarity_search(
                    query_embedding=query_embedding, k=5, artifact_types=query["types"]
                )
                ...
                query_result = {
                    ...
                    "top_artifact_type": (
                        results[0]["artifact_type"] if results else None
                    ),
                }
                ...
                logger.info(
                    f"  ✅ {query['description']}: {len(results)} results in {query_time:.1f}ms"
                )
                if results:
                    logger.info(
                        f"     🎯 Top match: {results[0]['artifact_type']} (score: {results[0]['similarity_score']:.3f})"
                    )
        ...
        if search_metrics["successful_searches"] > 0:
            search_metrics["average_search_time_ms"] = (
                search_metrics["total_search_time_ms"]
        ...
        logger.info(
            f"✅ Completed {search_metrics['successful_searches']}/{search_metrics['total_searches']} searches"
        )
        logger.info(
            f"⏱️  Average search time: {search_metrics['average_search_time_ms']:.1f}ms"
        )
        ...
        logger.info(
            f"💪 Stress test: {stress_searches} searches in {stress_test_time:.1f}ms"
        )
        logger.info(
            f"⚡ Search latency: avg={fallback_metrics['stress_test_avg_time_ms']:.1f}ms, min={fallback_metrics['stress_test_min_time_ms']:.1f}ms, max={fallback_metrics['stress_test_max_time_ms']:.1f}ms"
        )
        ...
        performance_results["overall_metrics"] = {
            ...
            "searches_per_second": (search_metrics["total_searches"] / total_test_time)
            * 1000,
        ...
        logger.info(
            f"📈 Document processing rate: {performance_results['overall_metrics']['documents_per_second']:.1f} docs/sec"
        )
        logger.info(
            f"🔎 Search rate: {performance_results['overall_metrics']['searches_per_second']:.1f} searches/sec"
        )
        logger.info(
            f"💾 Memory usage: {performance_results['overall_metrics']['memory_efficiency']['estimated_memory_mb']:.1f} MB"
        )
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(
            f"⏱️  Total Time: {results['overall_metrics']['total_test_time_ms']:.1f}ms"
        )
        exit(0)
    else:
        print(f"\n❌ Test Status: FAILED")
