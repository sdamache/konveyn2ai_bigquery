...
def test_fallback_functionality():
    ...
    try:
        ...
        from janapada_memory.models.vector_search_config import (
            VectorSearchConfig,
            DistanceType,
        )
        ...
        if results:
            logger.info(
                f"   - Best match: {results[0].chunk_id} (score: {results[0].similarity_score:.3f})"
            )

        # Test 2: BigQuery Vector Index with forced fallback
        logger.info("🔄 Testing BigQueryVectorIndex with forced fallback...")

        from janapada_memory.bigquery_vector_index import (
            BigQueryVectorIndex,
            SearchMode,
        )
        from janapada_memory.connections.bigquery_connection import (
        ...
        if fallback_results:
            logger.info(
                f"   - Best match: {fallback_results[0].chunk_id} (score: {fallback_results[0].similarity_score:.3f})"
            )
        ...
        memory_service = JanapadaMemoryService(
            connection=connection, config=config, enable_fallback=True
        )
        ...
        service_results = memory_service.similarity_search(
            query_embedding=query_embedding, k=2, artifact_types=["python"]
        )
        ...
        if service_results:
            logger.info(
                f"   - Best match: {service_results[0]['chunk_id']} (score: {service_results[0]['similarity_score']:.3f})"
            )
            logger.info(
                f"   - Metadata contains fallback flag: {service_results[0]['metadata'].get('fallback_search', False)}"
            )
