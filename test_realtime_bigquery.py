...
def test_realtime_bigquery_integration():
    ...
    try:
        ...
        logger.info(f"🔗 BigQuery connection healthy: {health['bigquery_connection']['is_healthy']}")
        logger.info(f"📋 BigQuery adapter status: {health['bigquery_adapter']['status']}")
        
        if health['bigquery_connection']['is_healthy']:
            logger.info("✅ BigQuery connection is working!")
        else:
            logger.warning(f"⚠️  BigQuery connection issue: {health['bigquery_connection']['error_message']}")
        ...
        try:
            ...
            if results:
                ...
                if results[0]['source'] == 'bigquery':
                    ...
                    logger.info("🔄 Fallback search activated (expected with empty tables)")
            else:
                ...
                if fallback_results:
                    logger.info(f"✅ Fallback search successful: {fallback_results[0]['chunk_id']}")
                    logger.info(f"🔄 Fallback metadata: {fallback_results[0]['metadata'].get('fallback_search', False)}")
                
        except Exception as search_error:
            logger.warning(f"🔄 Search encountered expected issue: {search_error}")
            logger.info("🧪 This is expected with empty BigQuery tables - fallback should activate")
