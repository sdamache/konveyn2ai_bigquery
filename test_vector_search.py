        ...
        try:
            ...
            if results:
                ...
                return {
                    ...
                    "first_embedding": (
                        row.first_embedding.isoformat() if row.first_embedding else None
                    ),
                    "last_embedding": (
                        row.last_embedding.isoformat() if row.last_embedding else None
                    ),
        ...
        try:
            ...
            if results:
                ...
                return {
                    ...
                    "text_content": (
                        row.text_content[:200] + "..."
                        if len(row.text_content) > 200
                ...
                similar_chunks.append(
                    {
                        ...
                        - float(row.distance),  # Convert cosine distance to similarity
                        "text_content": (
                            row.text_content[:200] + "..."
                            if len(row.text_content) > 200
                            else row.text_content
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"VECTOR_SEARCH returned {len(vector_search_results)} results"
                )
            except Exception as e:
            ...
            try:
                ...
                logger.info(
                    f"ML.APPROXIMATE_NEIGHBORS returned {len(approximate_neighbors_results)} results"
                )
            except Exception as e:
            ...
            if test_results.get("vector_search_results") or test_results.get(
                "approximate_neighbors_results"
            ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test BigQuery vector search functionality"
    )
    ...
    try:
        # Initialize tester
        tester = VectorSearchTester(project_id=args.project, dataset_id=args.dataset)
